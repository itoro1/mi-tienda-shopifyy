...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached raw transcript JSON (local faster-whisper)
...
- **`transcribe.py <video>`** — single-file local faster-whisper call, free, no API key. `--model tiny/base/small/medium/large-v3` (default `small`), `--diarize --num-speakers N` optional (needs free `HF_TOKEN`, see below). Cached.
- **`transcribe_batch.py <videos_dir>`** — parallel transcription, default 2 workers (CPU-bound, unlike a hosted API — raise `--workers` if you have the cores). Use for multi-take.
...
- **Audio events as signals.** `(laughs)`, `(sighs)`, `(applause)` mark beats. Extend past them. Note: the free local ASR (faster-whisper) doesn't tag non-speech audio events the way a hosted service does — spot these by ear/eye via `timeline_view.py` instead of scanning for them in the transcript text.
...
- **Defaulting to a paid transcription API.** This project runs `faster-whisper` locally on CPU on purpose — free, no key, no account. It's slower per file and `word_timestamps` output is close but not identical to a hosted service's, so pad cut edges a little more generously (Hard Rule 7) to absorb the extra drift. Bump `--model` up (e.g. `medium`) if accuracy on quiet or accented speech matters more than speed.
