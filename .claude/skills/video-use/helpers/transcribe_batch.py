...
on each (free, no API key), writes transcripts to
<videos_dir>/edit/transcripts/<name>.json.

Cached per-file: any source that already has a transcript is skipped.

Note: CPU inference is heavier than a hosted API. Default to fewer
workers than transcribe_batch used with a hosted service — 2 is a safer
default on a laptop CPU; raise it if you have the cores/RAM.
...
def main() -> None:
    ...
    ap.add_argument("--workers", type=int, default=2, help="Parallel workers (default: 2, CPU-bound)")
    ap.add_argument(
        "--model",
        type=str,
    ...
    print(f"transcribing {len(pending)} files with {args.workers} parallel workers "
          f"(faster-whisper/{args.model}, local, free)")
    t0 = time.time()
