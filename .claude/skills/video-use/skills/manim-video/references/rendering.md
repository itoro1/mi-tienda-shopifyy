...
Prefer free services — no API key, no paid account:

```bash
pip install "manim-voiceover[gtts]"     # Google Translate TTS, free, needs internet, decent quality (default choice)
pip install "manim-voiceover[pyttsx3]"  # fully offline, zero setup, robotic quality — use when there's no network at all
```

Do not reach for a paid TTS service (ElevenLabs, Azure, etc.) unless the user explicitly names one and provides their own key. Default to `gtts` for narration.
...
class NarratedScene(VoiceoverScene):
    def construct(self):
        self.set_speech_service(GTTSService(lang="en"))  # free, no key
...
Fully offline alternative (no internet at all, lower quality):
...
- Works with: Google TTS (free), pyttsx3 (free, offline), and paid services (Azure, ElevenLabs, etc.) if the user brings their own key — never assume one is available
